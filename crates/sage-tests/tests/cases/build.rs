...
fn observed_tools_contain_only_executed_compilers_and_linker() {
    ...
    assert_eq!(observed[0].parameters, ["CPPFLAGS=-DTEST=1", "CFLAGS=-O2"]);
...
fn mainstream_rclasses_expand_for_native_builds() {
    ...
    let kernel =
        Rclass::load(Path::new(env!("CARGO_MANIFEST_DIR")).join("../../rclass/kernel.toml"))
            .unwrap();
...
"#,
    ...
    sage::stage_sysusers(&recipe, &root.join(".METADATA"), "daemon", "system", "0").unwrap();
    assert_eq!(
...
fn elf_scanner_handles_non_elf_and_corrupt_files() {
    ...
    fs::write(
        directory.path().join("corrupted.so"),
...
fn private_runpaths_are_relative_and_passed_without_a_shell() {
    ...
    assert!(
        ElfScanner::rewrite_private_runpaths(&root, &[PathBuf::from("../lib")], &patchelf,)
            .is_err()
    );
}
