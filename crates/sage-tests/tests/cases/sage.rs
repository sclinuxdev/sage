...
async fn build_data_package(
    ...
    let conflicts = conflict.map_or(String::new(), |value| format!("conflicts=[\"{value}\"]\n"));
...
async fn signed_repository_lifecycle_recovers_and_preserves_configuration() {
    ...
    assert_ne!(
        sage_archive::inspect_package(&app1).unwrap().manifest.slot,
    ...
    .await;
    let aarch64 =
        load_available_with_pool(root.path(), Some("aarch64"), Some(pool.path())).unwrap();
    assert!(
        aarch64
            .releases
            .keys()
    ...
    assert!(
        sage_db::read_owners(&root.path().join("var/lib/sage"), "usr/bin/app-1")
            .unwrap()
            .is_empty()
    );
}
