...
use crate::models::{InstalledPackage, JournalRecord, decode_journal};
...
pub const MAP_SIZE: usize = 8 * 1024 * 1024 * 1024;

/// Named LMDB tables sharing one ACID environment.
pub struct SageDatabase {
    pub(crate) env: Env,
    pub(crate) packages: Database<Str, Bytes>,
    pub(crate) files: Database<Str, Bytes>,
    pub(crate) provides: Database<Str, Bytes>,
    pub(crate) system: Database<Str, Str>,
    pub(crate) operations: Database<Str, Bytes>,
...
pub(crate) fn encode<T: Serialize + ?Sized>(value: &T) -> Result<Vec<u8>, DbError> {
...
pub(crate) fn get_owned<T: DeserializeOwned>(
...
pub(crate) fn put_encoded<T: Serialize + ?Sized>(
    database: &Database<Str, Bytes>,
    txn: &mut RwTxn<'_>,
    key: &str,
    value: &T,
