...
- **全链路统一 LMDB 极速存储 (Unified LMDB Ecosystem)**:
  ...
  - **只读 mmap 与紧凑解码**: LMDB 通过 mmap 访问 B+ 树页；schema-v1 Bincode 值在点查或有序扫描后解码为 Rust 所有权对象。
...
| **全链路数据库 (Local & Remote)** | `heed` (LMDB Rust 绑定) | 单写多读、mmap B+ 树页与 ACID 事务。 |
| **归档与压缩** | `tar` + `zstd` | 单流低内存 ZSTD 解压与确定性归档。 |
