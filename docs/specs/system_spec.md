...
### 2.3 `[world].packages` 类似 Gentoo `@world` 语义与服务管理解耦
- `[world].packages`: 系统声明式常驻根软件包集合（置于 `system.toml` 最底部）：
  ...
  - **文件排版位置**：`[world]` 与其中的 `packages = [...]` 统一置于 `system.toml` 最底部，使系统基础元数据 (`[system]`) 与虚拟提供者映射 (`[providers]`) 一目了然置顶，同时遵循标准 TOML table scope。Schema v1 读取器仍兼容第一个 table header 之前的旧根级 `packages` 字段。
