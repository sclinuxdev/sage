...
`service.toml` may declare direct, socket, or system D-Bus activation. Native
unit files and activation descriptors are not duplicated in `data/`; the active
init rclass owns and regenerates those paths during `sage rebuild`.
